	...
	public function build(): \SimpleXMLElement
	{
		...
		foreach ($products as $item) {
			...
			if (!empty($product->meta_title)) {
				$xmlProduct->addChild('title', $product->meta_title);
			...
			$xmlProduct->addChild('description', $product->meta_description);

			$xmlCategories = $xmlProduct->addChild('categories');
			foreach ($product->getCategories() as $category_id) {
				$xmlCategories->addChild('category', (new \Category($category_id, $id_lang))->name);
			}

			$coverImage = \Product::getCover($product->id);
			$xmlProduct->addChild('image_link', $this->context->link->getImageLink($product->link_rewrite ?? $product->name, (int) $coverImage['id_image']));

			if (!empty($product->manufacturer_name) && 'Coins of Nakamoto' !== $product->manufacturer_name) {
				$xmlProduct->addChild('brand', $product->manufacturer_name);
			}
		}
